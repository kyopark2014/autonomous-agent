...
sharing_url = utils.get_sharing_url(config)
...
def execute_code(code: str) -> str:
    ...
    creating PowerPoint presentations, processing data, or performing
    computations. The execution environment has access to common libraries:
    reportlab, pypdf, pdfplumber, pandas, python-pptx, pptxgenjs (via
    subprocess/npx), Pillow, cairosvg, json, csv, os, requests, etc.

    PowerPoint files can be created directly using python-pptx or pptxgenjs.
    Always use this tool instead of telling the user to run code themselves.
...
def upload_file_to_s3(filepath: str) -> str:
    ...
    try:
        ...
        s3_bucket = utils.get_s3_bucket(config)
        if not s3_bucket:
            return "S3 bucket is not configured. Run installer.py to set up storage."
